...
uint8_t* GetImgDesc (uint8_t* buf, size_t len, lv_img_dsc_t_1* image_desc) {
    ...
    image_desc->dsc.header.magic = LV_IMAGE_HEADER_MAGIC;
    image_desc->dsc.header.cf = LV_COLOR_FORMAT_NATIVE_WITH_ALPHA;
    image_desc->dsc.header.flags = 0;
    image_desc->dsc.header.w = width;
    image_desc->dsc.header.h = height;
    image_desc->dsc.header.stride = width * LV_COLOR_NATIVE_WITH_ALPHA_SIZE;
    image_desc->dsc.data_size = width * height * LV_COLOR_NATIVE_WITH_ALPHA_SIZE;
    image_desc->dsc.data = img_data;
